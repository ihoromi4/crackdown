...
__all__ = [
    'RandomDistillationNetworkAgent',
]
...
class RandomDistillationNetworkAgent(Agent):
    def __init__(self,
                 env: gym.Env,
        ...
        self.rnd = RandomNetworkDistillation(self.embedding.shape[0])
    ...
    def train_batch(self, batch: list) -> dict:
        ...
        # final loss
        loss = self.critic_learning_rate * critic_loss + actor_loss + entropy_loss + rnd_loss
        ...
        self.report.add_scalar('curiosity', curiosity.mean().item(), global_step=self.iteration)
